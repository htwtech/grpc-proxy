...
pub trait ProxyHttp {
    ...
    /// Called after upstream TCP connection is established but BEFORE request
    /// headers are forwarded upstream. Allows async reading of the downstream
    /// request body (via `session.downstream_session.read_request_body().await`)
    /// and validation before any data reaches the upstream server.
    ///
    /// Returning `Ok(true)` means the request has been handled (e.g. a response
    /// has already been written downstream) and proxying should abort cleanly
    /// without contacting the upstream. Returning `Ok(false)` allows proxying
    /// to proceed normally. Returning `Err` aborts with an error.
    ///
    /// The implementation may store any body read in this hook in the context
    /// and re-inject it via [Self::request_body_filter()] so the upstream
    /// receives the original body on the happy path.
