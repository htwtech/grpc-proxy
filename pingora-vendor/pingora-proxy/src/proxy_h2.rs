    ...
    C: custom::Connector,
{
        ...
        SV::CTX: Send + Sync,
    {
        ...
        // Allow user code to inspect/validate the downstream request body
        // BEFORE any bytes are sent to the upstream. If the hook returns
        // Ok(true), the user has already written a response downstream and
        // we must abort without sending anything upstream.
        match self.inner.pre_upstream_body_filter(session, ctx).await {
            Ok(true) => {
                // Response already sent by user code; abort upstream cleanly.
                return (false, None);
            }
            Ok(false) => { /* continue */ }
            Err(e) => {
